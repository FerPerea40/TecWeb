      ...
      xml.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          if (respuesta.resultado == "Correcto") {
            alert("Orientador dado de baja");
            location.href = "baja_orientador.html";
          } else {
            alert("Error al dar baja");
          }
        }
      };
      xml.open("POST", "php/baja_orientador.php", true);
      const datos = document.querySelector("#form_login");
      xml.send(new FormData(datos));
    } else {
      alert("No se ha seleccionado ningun orientador");
    }
  },
  false
);
