...
function iniciar() {
  ...
  xml.open("POST", "php/revision_usuariosOrien.php", true);
...
document.getElementById("seleccionado").addEventListener(
  ...
  function (event) {
    ...
    if (document.getElementById("select").value != "0") {
      ...
      xml.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          if (respuesta.resultado == "Correcto") {
            user = respuesta.user;
            document.getElementById("Nom_Orient").value = respuesta.Nom_Orient;
      ...
      xml.open("POST", "php/editar_orientador_p1.php", true);
      const datos = document.querySelector("#form_login");
      xml.send(new FormData(datos));
    } else {
    ...
    xml.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (respuesta.resultado == "Correcto") {
          ...
          location.href = "editar_orientador.html";
        }
      }
    };
    xml.open("POST", "php/editar_orientador_p2.php", true);
    const datos = document.querySelector("#form_login");
    xml.send(new FormData(datos));
  },
  false
